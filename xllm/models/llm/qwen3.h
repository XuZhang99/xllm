...
// Loads the Qwen3 backbone args for the TORCH/Python path, transparently
// handling the vLLM "speculators" DSpark/DFlash draft config where the backbone
// architecture is nested under transformer_layer_config (flat plain-Qwen3
// configs still work via fallback). Kept as a free function so the comma-heavy
// key lists do not confuse the REGISTER_MODEL_ARGS macro's argument parsing.
// Mirrors load_qwen3_atb_args in models/llm/npu/qwen3.h.
inline void load_qwen3_args(const JsonReader& json, ModelArgs* args) {
  ...
  args->sliding_window() = json.value_or<int32_t>(tlc("sliding_window"), -1);
...
REGISTER_MODEL_ARGS(qwen3, [&] { load_qwen3_args(json, args); });
