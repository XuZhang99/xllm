...
XTensor::XTensor(const std::vector<int64_t>& kv_cache_shape,
                 torch::ScalarType dtype,
                 torch::Device& device)
    : dtype_(dtype) {
#if defined(USE_NPU)
  cache_tensor_ = at_npu::native::npu_format_cast(
      torch::empty(kv_cache_shape, torch::dtype(dtype_).device(device)), 2);
#elif defined(USE_MLU)
  cache_tensor_ =
      torch::empty(kv_cache_shape, torch::dtype(dtype_).device(device));
#endif
}
