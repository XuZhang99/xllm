...
def build_hisparse_gather_kernel(head_dim: int, vec_core_num: int) -> Any:
    """Read only misses from mapped Host memory; hot-cache hits stay on NPU."""
    ...
    def gather(
        ...
        hot: T.Tensor((MAX_ROWS, head_dim), "bfloat16"),
        slots: T.Tensor((MAX_ROWS,), "int32"),
        slot_map: T.Tensor((MAX_ROWS,), "int32"),
        tags: T.Tensor((MAX_ROWS,), "int32"),
        out: T.Tensor((MAX_ROWS, head_dim), "bfloat16"),
        rows: T.int32,
        host_rows: T.int32,
        hot_rows: T.int32,
    ):
                ...
                for iteration in T.serial((rows + vec_core_num - 1) // vec_core_num):
                    ...
                    if row < rows:
                        ...
                        if slot >= 0 and slot < host_rows:
                            cached = slot_map[slot]
                            if cached >= 0 and cached < hot_rows:
                                if tags[cached] == slot:
                                    T.copy(hot[cached, 0:head_dim], row_data)
                                else:
                                    T.copy(host[slot, 0:head_dim], row_data)
                            else:
                                T.copy(host[slot, 0:head_dim], row_data)
                        else:
                            T.tile.fill(row_data, 0)
                        T.copy(row_data, out[row, 0:head_dim])
