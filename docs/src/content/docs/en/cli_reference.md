...
| `enable_dsa_multi_stream` | `bool` | `false` | Overlap the DSA indexer and attention projections for the Python GLM model through the registered device stream API. Requires a supported model backend and stream-capable device; this option does not add model/kernel support for other devices. |
