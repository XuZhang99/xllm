...
void batch_decode(torch::Tensor& float_workspace_buffer,
  ...
  //   ffi::Array<int64_t> decode_plan_info =
  //   plan_info.cast<ffi::Array<int64_t>>(); std::cout << "decode_plan_info: "
  //   << decode_plan_info.size() << std::endl; for (int i = 0; i <
  //   decode_plan_info.size(); i++) {
  //     std::cout << "decode_plan_info[" << i << "]: " << decode_plan_info[i]
  //               << std::endl;
  //   }

  std::vector<int64_t> plan_info(10);
  plan_info[0] = 1;
  plan_info[1] = 0;
  plan_info[2] = 0;
  plan_info[3] = 0;
  ...
  plan_info[8] = 0;
  plan_info[9] = 0;
  ffi::Array<int64_t> decode_plan_info = ffi::Array<int64_t>(plan_info);
  ...
  std::cout << "batch_decode cuda run end" << std::endl;
}
