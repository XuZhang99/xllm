...
class QwenImageEditPlusPipelineImpl : public QwenImagePipelineBaseImpl {
 public:
  ...
  DiTForwardOutput forward(const DiTForwardInput& input) {
    ...
    /*
    if (prompt_embeds.size(1) % ::xllm::ParallelConfig::get_instance().sp_size()
    != 0) { int64_t pad_len =
          ::xllm::ParallelConfig::get_instance().sp_size() -
    prompt_embeds.size(1) % ::xllm::ParallelConfig::get_instance().sp_size();
    ...
    if (negative_prompt_embeds.size(1) %
    ::xllm::ParallelConfig::get_instance().sp_size() != 0) { int64_t pad_len =
    ::xllm::ParallelConfig::get_instance().sp_size() -
                        negative_prompt_embeds.size(1) %
    ::xllm::ParallelConfig::get_instance().sp_size(); std::vector<int64_t>
    pad_with = { 0, 0,  // 第3维�~Hhe   ight�~I�        ~Mpad 0, pad_len,  // 第
    2维�~Hchannels�~I�~I~M�~P~Npad 0, 0};  // 第1维�~Hbatch�~I�~Mpad
      std::vector<int64_t> pad_with_mask = {
          // 第3维�~Hhe   ight�~I�        ~Mpad
          0,
          pad_len,  // 第 2维�~Hchannels�~I�~I~M�~P~Npad
