...
namespace xllm {
void PageManagerServer::create_server(const memory::Options& options,
  ...
  auto page_manager_server =
      ServerHub::get_instance().register_server("DistributePageManagerServer");
...
PageManagerServer::PageManagerServer(int local_worker_idx,
                                     const std::string& master_node_addr,
                                     std::atomic<bool>& done,
                                     const torch::Device& device,
                                     const memory::Options& options) {
  page_manager_thread_ =
      std::make_unique<std::thread>(&PageManagerServer::create_server,
                                    this,
                                    std::cref(options),
                                    std::ref(done),
