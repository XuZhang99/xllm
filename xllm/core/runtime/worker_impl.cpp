...
folly::SemiFuture<std::optional<ForwardOutput>> WorkerImpl::step_async(
  ...
  // single-process mode each prepare can issue blocking CUDA driver calls
  // (cudaHostAlloc / cudaEventRecord through the CachingHostAllocator).
  // While one worker's step is busy-waiting on an NCCL collective for its
  // partner, those driver calls share libcuda's internal locks with the
  // running collective and the caller never returns to schedule the second
  // worker's step — both GPUs deadlock. Pinning prepare to the worker's
  // device thread keeps all CUDA work for that device on a single thread.
