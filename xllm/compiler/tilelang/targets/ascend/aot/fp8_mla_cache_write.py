...
from xllm.python.kernels_npu.tilelang import fp8_mla_cache_write as kernel_impl
from xllm.python.kernels_npu.tilelang import utils as tilelang_utils
from xllm.python.kernels_npu.tilelang.fp8_mla_cache_write import (
    SUPPORTED_DTYPES,
...
@register_kernel
class Fp8MlaCacheWriteKernel(TilelangKernel):
    DISPATCH_SCHEMA = [DispatchField("dtype", "dtype"), DispatchField("slot_bytes", "int32")]
    SPECIALIZATIONS = [
        {"variant_key": f"{dtype}_s{slot_bytes}", "dtype": dtype, "slot_bytes": slot_bytes}
        for dtype in SUPPORTED_DTYPES
        for slot_bytes in (4, 8)
    ]

    @staticmethod
    def generate_source(dtype: str, slot_bytes: int) -> str:
        tilelang.disable_cache()
        kernel = build_fp8_mla_cache_write_kernel(dtype, slot_bytes)
