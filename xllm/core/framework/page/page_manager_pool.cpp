...
#include "global_page_manager.h"
#include "option.h"
#include "server/xllm_server_hub.h"

namespace xllm {
PageManagerPool::PageManagerPool(const memory::Options& options,
                                 int32_t dp_size)
...
void PageManagerPool::setup_single_node_page_managers() {
  ...
  for (size_t i = 0; i < devices.size(); ++i) {
    ...
    page_managers_.emplace_back(
        std::make_unique<LocalPageManager>(options_, devices[i]));
    page_managers_clients_.emplace_back(
...
std::vector<size_t> PageManagerPool::num_free_pages_per_layer() const {
  ...
  for (int32_t i = 0; i < dp_size_; ++i) {
    num_free_pages_per_layer[i] = page_manager_clients_[i * dp_local_tp_size_]
                                      ->num_free_pages_per_layer();
  }
...
std::vector<size_t> PageManagerPool::num_used_pages_per_layer() const {
  ...
  for (int32_t i = 0; i < dp_size_; ++i) {
    num_used_pages_per_layer[i] = page_manager_clients_[i * dp_local_tp_size_]
                                      ->num_used_pages_per_layer();
  }
