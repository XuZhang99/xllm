...
| `kv_cache_dtype` | `string` | `"auto"` | KV Cache dtype for quantization. `auto` aligns with model dtype and disables quantization. `int8` is supported on existing quantized backends; `fp8_e4m3` stores PyTorch GLM-5.2 NPU caches in E4M3 format and dequantizes selected rows to BF16 for attention. |
