...
| `enable_hisparse` | `bool` | `false` | 为 GLM PyTorch/NPU 启用 Host 驻留的 BF16 稀疏 MLA KV；需禁用前缀缓存及调度重叠。 |
| `hisparse_device_buffer_size` | `int64` | `8192` | 每层、每个 worker 的 HiSparse 热 KV token 总容量，由全部请求共享。 |
| `hisparse_host_cache_size` | `int64` | `8589934592` | 每个 worker 跨全部层的完整 Host KV 字节上限。 |
