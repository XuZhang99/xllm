...
def test_glm_dsa_projections_precede_indexer(monkeypatch: pytest.MonkeyPatch, mode: str) -> None:
    ...
    class _Indexer(torch.nn.Module):
        def select_qli(self, *_args: object, **_kwargs: object) -> torch.Tensor:
            ...
            return torch.zeros(num_tokens, 1, cfg_values["index_topk"], dtype=torch.int32)

    hidden = torch.zeros(num_tokens, cfg_values["hidden_size"])
    attention.q_a_proj = _Projection(torch.zeros(num_tokens, cfg_values["q_lora_rank"]), "q_a")
    ...
    attention.kv_a_proj_with_mqa = _Projection(
        torch.zeros(num_tokens, cfg_values["kv_lora_rank"] + cfg_values["qk_rope_head_dim"]),
        "kv_a",
    )
    ...
    cp_context = SimpleNamespace(query_index=torch.arange(num_tokens)) if mode == "cp" else None
    if mode == "layerwise":
    ...
    monkeypatch.setattr(
        glm5_2.kernels,
        "atb_matmul_ein_sum",
        lambda lhs, rhs: torch.einsum("thd,hdo->tho", lhs, rhs),
        raising=False,
    )
...
def test_glm_indexer_projection_overlap_matches_serial(
    ...
    if fused_projection:
        with patch.object(indexer.wq_b, "process_weights_after_loading", create=True):
    ...
    class _Stream:
        ...
        def record_on_current(self, tensor: torch.Tensor) -> None:
            order.append(self.name + "_record")
    ...
    original_linear = glm5_2.F.linear

    def _linear(value: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor | None = None) -> torch.Tensor:
        if weight is indexer._wk_weights_proj_weight:
            order.append("fused_" + active[0])
        return original_linear(value, weight, bias)

    monkeypatch.setattr(glm5_2.F, "linear", _linear)
    ...
    def _rope(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, start: int, dim: int) -> torch.Tensor:
        ...
        even, odd = value[..., 0::2].clone(), value[..., 1::2].clone()
        value[..., 0::2] = even * c - odd * s
        value[..., 1::2] = odd * c + even * s
        return x
    ...
    if device_type == "cpu":
        if fused_projection:
            assert order.count("fused_weights") == 1
            assert "weights_weights" not in order
            assert "k_weights" not in order
        else:
            assert "weights_weights" in order
            assert "k_weights" in order
        assert order.index("weights_join") < order.index("cache") < order.index("select")
        assert order.count("weights_record") == 2
    ...
    graph_state = AclGraphExecutionState({})
