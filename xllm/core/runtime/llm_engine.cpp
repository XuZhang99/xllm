...
bool LLMEngine::allocate_continuous_kv_cache(
  ...
  xtensor_options.num_kv_heads(n_local_kv_heads_)
      .head_size(head_dim)
      .max_context_len(args_.max_position_embeddings())
      .max_seqs_per_batch(options_.max_seqs_per_batch())
      .granularity_size(options_.granularity_size())
      .dtype(dtype_);
  ...
  for (auto& worker : worker_clients_) {
    futures.push_back(
        worker->allocate_continuous_kv_cache_async(xtensor_options));
  }
  // wait for all futures to complete
  auto results = folly::collectAll(futures).get();
  for (const auto& result : results) {
  ...
  const int64_t cache_size_per_token = 0;
  if (FLAGS_enable_mla) {
    cache_size_per_token = kv_cache_cap.slot_size;
  } else {
  ...
  memory_options.devices(options_.devices())
      .granularity_size(FLAGS_granularity_size)
      .total_pages(kv_cache_cap.n_pages)
      .num_layers(args_.n_layers())
      .cache_size_per_token(cache_size_per_token);
  page_manager_pool_ =
      std::make_unique<PageManagerPool>(memory_options, options_.dp_size());
