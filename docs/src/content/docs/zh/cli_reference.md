...
| `kv_cache_dtype` | `string` | `"auto"` | KV Cache 量化数据类型；`auto` 表示与模型 dtype 对齐且不量化，`int8` 用于已有量化后端，`fp8_e4m3` 将 NPU PyTorch GLM-5.2 cache 存储为 E4M3 格式，并在 attention 计算前将选中数据反量化为 BF16。 |
